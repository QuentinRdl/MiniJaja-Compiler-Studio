...
\begin{titlepage}
    ...
    \begin{center}
        ...
        {\Large Islem \textsc{HADJ RABIA}, Théo \textsc{HUMBERT}, Laura \textsc{JACQUESON}, Kilian \textsc{JELIC}, Julie \textsc{MAGNIN}, Quentin \textsc{RADLO}}\\[4cm]
...
Durant le premier semestre du Master d’informatique à l’Université Marie et Louis Pasteur de Besançon, nous devons réaliser un projet commun entre les enseignements de compilation et de génie logiciel. Répartis en groupes de six étudiants, nous disposons de la période de septembre à décembre pour le mettre en place. Ce projet a pour objectif de mettre en pratique les connaissances acquises en compilation tout en appliquant la méthode Agile, abordée dans le cours de génie logiciel. Il consiste à développer un interpréteur et un compilateur accompagné d’une interface utilisateur. Ce compilateur s’applique au langage MiniJaja, un langage simplifié et inspiré de Java.

Ce rapport se découpe en trois grandes parties. La première partie présentera le bilan de la release 1, en détaillant les objectifs de cette release et des sprints, la gestion des tâches et des User Stories, ainsi que le bilan des sprints, incluant les rétrospectives, le point de vue de l’équipe, les points positifs et les axes d’amélioration du projet. La deuxième partie sera consacrée à la qualité du code et à l’intégration, en abordant les tests unitaires et d’acceptation, les revues ainsi que l’intégration et le déploiement continu. Enfin, la troisième partie traitera de l’organisation du projet, en évoquant la coordination et la communication au sein de l’équipe. Le rapport se conclura par une prévision de notre méthode de travail pour la release 2.
...
\subsubsection{Objectifs des sprints}
...
Nous avons choisi de rédiger nos \emph{User Stories} au cours de multiples réunions organisées durant le premier sprint.
Nous avons commencé par les écrire selon le modèle :
\begin{quote}
\textit{« En tant que …, je veux pouvoir … afin de … »}
\end{quote}

Nous les avons ensuite peu à peu raffinées, d’abord en découpant les \emph{User Stories} les plus volumineuses en plus petites, puis en leur donnant des critères d’acceptation, ainsi que les tests d’acceptation
associés.
...
Pour évaluer la complexité de chaque tâche, nous nous réunissions avant chaque début de sprint pour analyser les tâches prévues. Nous discutions de leur difficulté technique et de la charge de travail qu’elles pouvaient représenter sur l’ensemble du sprint.

À l’issue de cette concertation, nous estimions le temps de travail nécessaire pour chaque tâche, et nous avons convenu de l’approche suivante : chaque heure de travail estimée correspondait à un \emph{story point}. Ainsi, si nous estimions qu’une tâche devait prendre cinq heures, nous renseignions le champ \emph{Story Point} de cette tâche sur Jira à 5. Cas particulier : si une tâche était estimée à moins d’une heure de travail, nous fixions malgré tout son champ à 1.

Cette approche nous a permis d’obtenir des estimations pas toujours réalistes, mais au moins homogènes pour chaque tâche, et donc de répartir au mieux le travail entre les différents membres de l’équipe, afin que la charge de travail de chacun soit la plus juste possible. De cette manière, nous avons également pu planifier l’avancement du projet sur chaque sprint et vérifier si nous étions en avance ou en retard.
...
Avant le début de chaque sprint, nous organisions une réunion de planification pour définir les tâches à réaliser, évaluer leur complexité et déterminer leur priorité. Nous estimions le temps nécessaire pour chaque tâche et accordions une priorité plus élevée aux tâches les plus longues et les plus importantes. Nous prenions également en compte les dépendances : certaines tâches devaient être réalisées en premier pour ne pas bloquer le reste du sprint.

Par ailleurs, certaines tâches moins essentielles ou rapides à réaliser étaient programmées en dernier, ce qui nous permettaient d’optimiser notre temps et de maintenir une progression continue tout au long du sprint.

Une fois les priorités définies, nous les reportions dans le tableau Jira, en utilisant les niveaux de priorité : Très urgent (\emph{Highest}), Urgent (\emph{High}), Moyen (\emph{Middle}), Faible (\emph{Low}) et Très faible (\emph{Lowest}). Ce fonctionnement a permis à l’équipe de savoir sur quelle action se concentrer en priorité.

\subsection{Bilan des sprints}
\subsubsection{Rétrospectives des sprints}

À la fin des sprints 2 et 3, nous avons réalisé des rétrospectives de sprint. Celles-ci consistaient à revenir sur le sprint écoulé, en soulignant les points positifs et les points à améliorer afin de progresser pour les sprints suivants.

\paragraph{Rétrospective du sprint 2}

Tous les membres étaient présents, excepté Islem arrivé en fin de réunion. Nous avons relevé les points suivants :

\textbf{Points à améliorer :}
\begin{itemize}
    \item Mieux découper les \emph{Merge Requests} ;
    \item S’assigner à une \emph{Merge Request} lors de sa création ;
    \item S’attribuer en tant que \emph{Reviewer} lors d’une \emph{Code Review} ;
    \item Approuver la \emph{Merge Request} avant de la fusionner pour faciliter la gestion \emph{Git} ;
    ...
    \item Une \emph{Merge Request} ne doit pas être trop grande ni trop petite, à juger selon le cas ;
...
\begin{itemize}
    ...
    \item Revues de code bien gérées ;
...
À la fin de chaque rétrospective, nous recueillions également le point de vue de chaque membre, avec leurs ressentis sur le sprint qui vient de s’écouler.
...
\begin{itemize}
    ...
    \item \textbf{Kilian} : Partage l'avis du reste du groupe. Il se dit particulièrement satisfait de la présence et de la réactivité de tous les membres. Il pense que le résultat final répondra aux attentes initiales.

    \item \textbf{Islem} : Trouve que les sprints se sont très bien déroulés et se montre confiant pour la suite du projet.

    \item \textbf{Laura} : Se montre moins confiante, car certains objectifs du sprint 3 n'ont pas été atteints, ce qui l'inquiète pour la fin du projet. Cependant, elle se dit satisfaite du bon fonctionnement du groupe, malgré son absence partielle lors de la fin du sprint 3, due à des problèmes personnels.
\end{itemize}

\subsubsection{Points positifs du projet}

Nous avons pu réaliser de nombreuses \emph{Merge Requests}, ainsi tout le code écrit a été revu et validé. Nous avons donc obtenu un code de bonne qualité, bien ordonné, correctement documenté et testé.

La répartition des tâches a été appropriée, personne ne semblait trop en faire ou pas assez, et le projet a pu avancer comme on le souhaitait.

Nous avons maintenu une très bonne communication, ce qui nous a permis de résoudre rapidement et efficacement les problèmes rencontrés.
...
La mise en place d'une architecture, dès le premier sprint, nous a permis de réaliser des tâches différentes simultanément, ce qui a rendu notre travail plus efficace. L'intégration entre les modules s'est déroulée sans difficulté.

\subsubsection{Points à améliorer}

La gestion des tâches sur Jira a été difficile en raison de la non-intuitivité du logiciel. En effet, nous avions décidé de découper les tâches en sous-tâches et de les estimer. Malheureusement, toutes les sous-tâches n'étaient prises en compte qu'une fois toutes les sous-tâches achevées. En conséquence, le graphique \emph{Burndown} comporte de nombreux pics descendants, et la vélocité de l'équipe est difficile à déterminer.

Nous avons mal pris en compte nos emplois du temps et avons été confrontés à une charge de travail trop importante par rapport au temps dont nous disposions. Cela est particulièrement visible au sprint 3, où nous n'avons pas pu terminer tout ce qui était prévu à cause des révisions de plusieurs devoirs.

Nous avons également sous-estimé la difficulté de certaines tâches, qui ont finalement demandé bien plus de temps que prévu.
...
Chaque module possède son propre répertoire de tests. Cette organisation facilite la maintenance, améliore la lisibilité et permet une détection rapide des erreurs. De plus, chaque test est indépendant des autres afin d'éviter toute interférence entre eux.

L'exécution des tests se fait via \textbf{Maven}, assurant l'intégration des tests dans le build. Cette méthode permet de vérifier rapidement le bon fonctionnement du projet et de détecter d'éventuelles régressions après chaque modification du code.
...
De cette manière, nous pouvions également vérifier les tests ajoutés, mais plus globalement, nous contrôlions que la couverture totale de notre projet avançait conjointement avec les nouvelles fonctionnalités apportées grâce à l’analyse SonarQube.

Aussi, après chaque sprint, nous organisions une revue de sprint. Avec toute l’équipe présente, nous validions les différentes fonctionnalités terminées, ajustions les priorités du \emph{backlog} et recueillions les ressentis de chaque membre de l’équipe.

\subsection{Intégration et déploiement continu}
Pour le projet, nous avons exploité différentes techniques d’intégration et de déploiement continu afin d’automatiser, autant que possible, la validation et la mise à disposition de notre code.

Premièrement, nous avons mis en place une pipeline CI/CD via GitLab. Celle-ci était exécutée à chaque Merge Request et constituait une étape obligatoire avant toute fusion vers la branche \emph{dev}. Cela nous garantissait que le code présent sur \emph{dev} était bien fonctionnel, car la pipeline réalisait automatiquement la compilation et l’exécution des tests, en indiquant clairement si tout s'était déroulé avec succès ou non.

Ensuite, un job de cette pipeline envoyait les résultats de nos tests à SonarQube. De cette manière, nous avions toujours un aperçu de ce que nos tests couvraient dans notre code. Grâce à cela, nous avons pu identifier les modules bien testés et ceux nécessitant davantage de couverture.

Enfin, par le biais de Nexus, nous avons généré différentes \emph{snapshots}, pour toujours garder une trace de l’avancement de notre projet.

\newpage

\section{Organisation}
\subsection{Coordination de l'équipe}
Dès le début du projet, notre équipe s’est organisée efficacement. Nous avons défini un fonctionnement inspiré de la méthode \emph{Agile}, avec une répartition claire et une communication régulière. Des réunions hebdomadaires nous permettaient de faire le point sur les avancées de chacun, d’identifier les difficultés et d’ajuster les tâches si nécessaire. En complément, nous faisions régulièrement des points rapides sur \textbf{Discord} afin de tenir l’équipe informée du travail en cours. Ces échanges réguliers nous ont permis de savoir en permanence où en était le projet, tout en donnant à chacun l’occasion de s’exprimer.
...
Pour cette deuxième release, nous allons poursuivre notre méthode de travail, que nous trouvons efficace et adaptée à notre équipe. Nous créons une branche par fonctionnalité, ce qui facilite les \emph{Merge Requests} et rend l’intégration du code plus simple. Chaque \emph{Merge Request} est accompagnée d’une documentation claire avec une \emph{Javadoc} et les tests associés au code couvrent une grande partie des fonctionnalités développées.

Nous continuerons également à communiquer régulièrement, comme lors de cette première release, avec des réunions hebdomadaires et des points réguliers sur Discord. Nous avons constaté que cette manière de travailler nous permettait de maintenir un code de qualité, sans blocages liés à la compréhension du travail des autres. De plus, ces échanges réguliers ont renforcé notre cohésion et notre dynamique d’équipe.
